...
```{r seed_plot_peparations, include=F}
...
# Function for making a dataframe of row/column names to save to a file in case the plot labels are not readable
make_row_col_mat <- function(mat) {
    row_col_max_length <- max(c(length(rownames(mat)), length(colnames(mat))))
    return(data.frame(Row.Names=c(rownames(mat), rep(NA, row_col_max_length-length(rownames(mat)))),
                          Column.Names=c(colnames(mat), rep(NA, row_col_max_length-length(colnames(mat))))))
}
```
...
par(oma=c(3,3,3,3))
dist_heatmap <- heatmaply(sampleDistMatrix, dendrogram = "both", col=colors, grid_color = "grey", fontsize_row=label_size(nrow(sampleDistMatrix)), fontsize_col=label_size(ncol(sampleDistMatrix)), show_rownames=label_size(nrow(sampleDistMatrix))>=4, show_colnames=label_size(ncol(sampleDistMatrix))>=4)
export(dist_heatmap, file="differential_gene_expression/plots/Heatmaps_of_distances.png")
export(dist_heatmap, file="differential_gene_expression/plots/Heatmaps_of_distances.pdf")
#dend <- hclust(dist(mtcars, method = "euclidean"), method = "complete")
#capture.output(cutree(dend, k = 3))
#write.table(make_row_col_mat(sampleDistMatrix), file="differential_gene_expression/plots/Heatmaps_of_distances_index.tsv", row.#names=F, quote=F)
```
...
pg <- ggplot(DE_all, aes(x=log2FoldChange, y=logpval, text=paste("Gene: ", gene_name, "<br>", "Log2FC: ", formatC(log2FoldChange, digits=2)))) +
        ...
        geom_point(alpha=0.5, aes(color=ifelse(abs(DE_all$log2FoldChange) > params$logFC_threshold &  DE_all$logpval > -log10(params$pval_threshold), "Differentially expressed genes", "Non-differentially expressed genes"))) +
        geom_hline(yintercept = 16, linetype= "dashed", size = 0.2, color = "grey") +
        geom_hline(yintercept = -log10(params$pval_threshold), size = 0.2, color = "grey") +
...
pg[['x']][['layout']][['annotations']][[2]][['x']] <- -params$pval_threshold #-0.05 TODO: what do these two lines do?
pg[['x']][['layout']][['annotations']][[1]][['y']] <- -params$pval_threshold
...
if (isProvided(params$custom_gmt)){
    ...
    tryCatch(
        ...
        error=function(gost_error) {
            ...
            print("Got error:")
            print(gost_error)
            print("For reproducibility reasons, try to download the GMT file manually by visiting https://biit.cs.ut.ee/gprofiler/gost, then selecting the correct organism and finally, in datasources, clicking 'combined ENSG.gmt'. Then provide it to the pipeline with the parameter `--custom_gmt`")
        }
    )

    # Also try to get the version info from the current build
    gpro_version_url <- paste0("https://biit.cs.ut.ee/gprofiler/api/util/data_versions?organism=", organism)
    gpro_version_string <- ""
    tryCatch(
        {
            gpro_command <- paste0("curl ", gpro_version_url)
            sys_return <- system(gpro_command)
            if ("gprofiler_version" %in% sys_return) {
                info <- sys_return
                sources <- info$sources
                gpro_version_string <- paste0("`gprofiler: ", info$gprofiler_version, "`, ")
            } else {
                print("Failed to obtain the gprofiler version with this command:")
                print(gpro_command)
                print("For reproducibility reasons, find the gprofiler command on https://biit.cs.ut.ee/gprofiler/gost.")
            }
        },
        error=function(version_error) {
            print("Failed to fetch the gprofiler version with this command:")
            print(gpro_command)
            print("Got error:")
            print(version_error)
            print("For reproducibility reasons, find the gprofiler command on https://biit.cs.ut.ee/gprofiler/gost.")
            ...
            if (nrow(df) <= 100 & nrow(df) > 0) {
                ...
                for (i in c(1:nrow(df))){
                    ...
                    if (!isProvided(params$citest) & nrow(mat)>1){
                        ...
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)), show_rownames=(label_size(nrow(mat))>=4), show_colnames=(label_size(ncol(mat))>=4))
                        dev.off()
                        ...
                        pheatmap(mat = mat, annotation_col = metadata_save, main = paste("Heatmap of normalized expression values for genes in pathway ", pathway$short_name, "(",pathway$source,")",sep=" "), scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)), show_rownames=(label_size(nrow(mat))>=4), show_colnames=(label_size(ncol(mat))>=4))
                        dev.off()
                        write.table(make_row_col_mat(mat), file=paste("pathway_analysis", "/", fname, "/", "pathway_heatmaps", "/", "Heatmap_normalized_counts_", pathway$source, "_", pathway$term_id, "_", fname, "_index.tsv", sep=""), row.names=F, quote=F)
                    }
...
if (isProvided(params$path_genelist)){
    ...
    if (nrow(mat)>1){
        ...
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific heatmap of normalized expression values for genes", scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)), show_rownames=(label_size(nrow(mat))>=4), show_colnames=(label_size(ncol(mat))>=4))
        dev.off()
        pdf(paste("pathway_analysis", "/", genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list.pdf", sep=""))
        pheatmap(mat = mat, annotation_col = metadata_save, main = "Genelist-specific heatmap of normalized expression values for genes", scale = "row", cluster_cols = F, cluster_rows = T, fontsize_row=label_size(nrow(mat)), fontsize_col=label_size(ncol(mat)), show_rownames=(label_size(nrow(mat))>=4), show_colnames=(label_size(ncol(mat))>=4))
        dev.off()
        write.table(make_row_col_mat(mat), file=paste("pathway_analysis", "/", genelist_heatmaps_dir, "/", "Heatmap_normalized_counts_gene_list_index.tsv", sep=""), row.names=F, quote=F)
    }
}
```
...
```{r software_versions, bootstrap.show.message=FALSE, echo=FALSE, message=FALSE}
# Import the .csv or .yml file output by the rna-seq Nextflow pipeline
is_csv <- endsWith(params$path_software_versions, ".csv") || endsWith(params$path_software_versions, ".tsv")
if (is_csv) {
...
The Nextflow-based nf-core pipeline `rnaseq `r as.character(ifelse(is_csv, version["nf-core/rnaseq"], version["Workflow"][[1]]["nf-core/rnaseq"]))`` [^1] was used for the RNAseq Bioinfomatics analysis. `FASTQC `r as.character(ifelse(is_csv, version["FastQC"], version["FASTQC"][[1]]["fastqc"]))`` [^3] [@andrews2010fastqc] was used to determine quality of the FASTQ files.
Subsequently, adapter trimming was conducted with `Trim Galore `r as.character(ifelse(is_csv, version["Trim Galore!"], version["TRIMGALORE"][[1]]["trimgalore"]))`` [^4] [@krueger2012trim]. `STAR v`r as.character(ifelse(is_csv, substring((version["STAR"]), 7, ), version["MAKE_TRANSCRIPTS_FASTA"][[1]]["star"]))`` [@Dobin2013] aligner was used to map the reads that passed the quality control to the reference genome.
The RNA-seq data quality control was performed with `RSeQC `r as.character(ifelse(is_csv, version["RSeQC"], version["RSEQC_BAMSTAT"][[1]]["rseqc"]))`` [@wang2012rseqc] and read quantification of the features (e.g. genes) with
```{r quant_cite, echo=FALSE, results='asis', eval=T}
cat(paste0("`", quant_tool, " ", quant_version, "` ", quant_cite, "."))
```
```{r MQC_cite, echo=FALSE, results='asis', eval=is_csv}
...
The differential expression analysis was performed by the use of the `rnadeseq `r as.character(params$revision)`` pipeline [^5], which was written using the nf-core template [@ewels2020nf]. The pipeline was run with the following command:  
``r params$commandline``  
For differential expression analysis, the read quantification data resulting from `r quant_tool` were processed with the R package `DESeq2 v`r packageVersion("DESeq2")`` [@love2014differential].
...
if (isProvided(params$custom_gmt)){
    ...
    if (gpro_version_string != "") {
        database_string <- paste0(database_string, gpro_version_string, ",")
    }
    database_version_info <- get_version_info(params$organism)
    for (db in datasources) {
        database_string <- paste0(database_string, "`", db, ": ", gsub("\n", ", ", database_version_info$sources[[db]]$version), "`, ")
    }
    database_string <- paste0(strsplit(database_string, ",\\s*(?=[^,]+$)", perl=TRUE)[[1]], ".")
}

cat(paste0("For pathway analysis, the R packages `gprofiler2 v", version_gprofiler2," `, `AnnotationDbi v", version_annotation, "` and `", name_species, " v", version_annotation, "` were employed. ", database_string))
```
